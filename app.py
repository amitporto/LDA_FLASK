...
def fit_linear_reg(X,y):
    ...
    return round(Wilks_lambda,3),round(F_value,3),p_value
...
def data():
    if request.method == 'POST':
       ...
       return render_template('data.html', tptr=tptr,tntr=tntr, fptr=fptr,fntr=fntr,acctr=acctr,
              ...
              trsize=ntr.shape[0],tssize=nts.shape[0], mc=mc,
              Wilks_lambda=Wilks_lambda,F_value=F_value,p_value=p_value, tbl=desTable.to_html(),eq=eq)
